...
(function(){
  ...
  function testDownload(ms=3500, parallel=3){ let loaded=0; const start=performance.now(), end=start+ms; let dlEMA=null; function run(){ return new Promise((resolve)=>{ (function loop(){ if(performance.now()>=end){ resolve(); return; } const size=1024*1024; const remaining=end-performance.now(); if(remaining<=0){ resolve(); return; } const c=('AbortController' in window)?new AbortController():null; const abortT=c?setTimeout(()=>c.abort(),remaining):null; fetch(CF_DOWN+size,Object.assign({cache:'no-store',mode:'cors'},c?{signal:c.signal}:{})) .then(r=>{ if(r.body&&r.body.getReader){ const reader=r.body.getReader(); return (function pump(){ return reader.read().then(({done,value})=>{ if(done) return; if(value) loaded+=value.byteLength*8; if(performance.now()>=end){ reader.cancel(); return; } return pump(); }).catch(()=>{}); })(); } return r.arrayBuffer().then(b=>{ if(b&&b.byteLength) loaded+=b.byteLength*8; }); }) .catch(()=>{}) .finally(()=>{ if(abortT) clearTimeout(abortT); const elapsed=(performance.now()-start)/1000; const mbps=bitsToMbps(loaded/Math.max(elapsed,0.001)); dlEMA=dlEMA==null?mbps:(dlEMA*0.8+mbps*0.2); line.push(dlEMA); loop(); }); })(); }); } const ps=[]; for(let k=0;k<parallel;k++) ps.push(run()); return Promise.all(ps).then(()=>{ const elapsed=(performance.now()-start)/1000; return {downMbps:bitsToMbps(loaded/Math.max(elapsed,0.001))}; }); }
